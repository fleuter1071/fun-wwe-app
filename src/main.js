...
function getImage(event) {
  return event.strThumb || event.strFanart || "https://via.placeholder.com/1200x675/101724/FFFFFF?text=WWE+Event";
}
...
function renderList(events) {
  ...
  eventListEl.innerHTML = events.map((event) => {
    ...
    const active = event.idEvent === selectedEventId ? "active" : "";

    return `
      <article class="event-card ${active}" data-id="${event.idEvent}">
        <img class="thumb" src="${getImage(event)}" alt="${safe(event.strEvent)}" />
        <div>
          <h3 class="event-title">${safe(event.strEvent)}</h3>
          <div class="meta">${formatDate(event.dateEventLocal || event.dateEvent)} • ${formatTime(event.strTimeLocal || event.strTime)}</div>
          <div class="meta">${safe(event.strVenue)} • ${safe(event.strCity)}</div>
          <div class="chips">
            <span class="chip ${statusClass}">${parseStatus(event.strStatus)}</span>
            <span class="chip">?? ${matchCount} matches</span>
            <span class="chip">?? ${titleMatchCount} title matches</span>
            ${titleChanges ? `<span class="chip title-change">? ${titleChanges} title change${titleChanges > 1 ? "s" : ""}</span>` : ""}
          </div>
        </div>
      </article>
    `;
  ...
  document.querySelectorAll(".event-card").forEach((card) => {
    card.addEventListener("click", () => {
      ...
      renderDetail(event);
      if (window.innerWidth <= 760) shellEl.classList.add("mobile-detail-active");
    });
  });
}
...
function renderDetail(event) {
  ...
  detailContentEl.innerHTML = `
    <div class="detail-top">
      <div>
        <div class="breadcrumb">Events / ${safe(event.strEvent)}</div>
        <h2>${safe(event.strEvent)}</h2>
        <small>${formatDate(event.dateEventLocal || event.dateEvent)} • ${formatTime(event.strTimeLocal || event.strTime)}</small>
      </div>
      <div class="detail-actions">
        <button id="prevEventBtn" class="btn btn-small mobile-only">? Events</button>
        <button id="randomDetailBtn" class="btn btn-small">?? Random</button>
      </div>
    </div>

    <div class="detail-body">
      <section class="hero-banner">
        <img src="${getImage(event)}" alt="${safe(event.strEvent)}" />
        <div class="hero-overlay">
          <div>
            <h3>${safe(event.strEvent)}</h3>
            <p>${safe(event.strVenue)} • ${safe(event.strCity)}, ${safe(event.strCountry)}</p>
          </div>
      ...
      <section class="snapshot-grid">
        ...
        <div class="snapshot">
          ...
          <div class="snapshot-value">${matchCount} matches • ${titleMatchCount} title matches</div>
        </div>
        <div class="snapshot">
      ...
      <section class="block">
        <div class="block-header">
          <div class="block-title-wrap">
            ...
            <p>Use one switcher here to move between the announced card and the actual results.</p>
          </div>
      ...
      <section class="block">
        <div class="block-header">
          <div class="block-title-wrap">
            <h3>Media & Notes</h3>
            <p>Quick access to related video and a note about how the match stats are detected.</p>
          </div>
        </div>
        <div class="block-body">
...
function applyFilters() {
  ...
  if (selectedEvent) {
    ...
    detailContentEl.innerHTML = '<div class="detail-empty">No event is currently selected because nothing matches your filters.</div>';
  }
}
